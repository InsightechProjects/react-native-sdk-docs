...
**Tested with:** React Native 0.68 through 0.81, React 17 through 19, both Paper and Fabric renderers. React Native 0.76–0.81 are verified on every push by the SDK's CI matrix (TypeScript build + full unit-test suite, against each version's bundled types and its matching React 18/19). React Native 0.81 (React 19, New Architecture/Fabric) is additionally verified end-to-end on the iOS simulator via the `example/` app. The 0.77–0.80 on-device matrix and Android device runs are tracked for follow-up.
